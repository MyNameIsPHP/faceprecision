...
class RaceSkintoneClassifier:
    def __init__(self, race_classifier_model_path, skintone_classifier_model_path):
        self.session = onnxruntime.InferenceSession(race_classifier_model_path)
        self.input_name = self.session.get_inputs()[0].name

    def preprocess_image(self, img):
        """Converts the image to the required format for the model."""
        rgb_img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
        resized_img = cv2.resize(rgb_img, (80, 80))
    ...
    def predict(self, img):
        ...
        mask_result = np.argmax(output, axis=1)
        return ["saiyan", "nguoi da den"]
